...
const headCells = [
    ...
    { id: "detail", label: "Description", width: 350 },
...
export default function CEO() {
    ...
    const fetchCEOs = async () => {
        try {
            ...
            const res = await fetch(`${import.meta.env.VITE_API_URL}/ceo/`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            if (!res.ok) throw new Error("Failed to fetch CEO");
    ...
    return (
        <Box sx={{ width: "100%" }}>
            ...
            <ActiveLastBreadcrumb label={"CEO"} endpoint={"List CEO"} />
            <Paper sx={{ width: "100%", mb: 2 }}>
                <Header name={"CEOs"} description={"Manage your CEOs here."} />


                <Box sx={{ padding: '5px 16px' }}>
                    <Link to="/ceo/add" style={{ textDecoration: "none" }}>
                        <Button variant="contained">Create CEO</Button>
                    </Link>
                </Box>
                            ...
                            {visibleRows.map((row) => (
                                <TableRow hover key={row.id}>
                                    <TableCell>
                                        <ActionEdit
                                            onEdit={() =>
                                                navigate(`/ceo/edit/${row.id}`)
                                            }
                                            message="Edit CEO"
                                        />
                                        <ActionDelete
                                            link={`${import.meta.env.VITE_API_URL}/ceo/delete/${row.id}`}
                                            onDelete={fetchCEOs}
