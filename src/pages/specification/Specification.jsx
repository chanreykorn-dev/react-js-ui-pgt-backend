...
export default function Specification() {
    ...
    const fetchCategories = async () => {
        try {
            ...
            const res = await fetch(`${import.meta.env.VITE_API_URL}/spicifications/`, {
            ...
            <Paper sx={{ width: "100%", mb: 2 }}>
                ...
                <Box sx={{ padding: "5px 16px" }}>
                    {hasPermission("Create Categories") ? (
                        <Link to="/specification/add" style={{ textDecoration: "none" }}>
                            <Button variant="contained">Create Specification</Button>
                        </Link>
                    ) : (
                        ...
                        <TableBody>
                            {visibleRows.map((row) => (
                                <TableRow hover key={row.id}>
                                    <TableCell>
                                        <ActionEdit
                                            onEdit={() => navigate(`/specification/edit/${row.id}`)}
                                            message="Edit Specification"
                                            disabled={!hasPermission("Update Spicifications")}
                                        />
                                        <ActionDelete
                                            link={`${import.meta.env.VITE_API_URL}/spicifications/delete/${row.id}`}
                                            onDelete={fetchCategories}
                                            message="Specification deleted successfully"
                                            disabled={!hasPermission("Delete Spicifications")}
                                        />
                                    </TableCell>
