...
const headCells = [
    ...
    { id: "detail", label: "Detail", width: 300 },
...
export default function Welcome() {
    ...
    const fetchWelcome = async () => {
        try {
            ...
            const res = await fetch(`${import.meta.env.VITE_API_URL}/welcome/`, {
            ...
            console.error(err);
            toast.error("Error loading welcome");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <Box sx={{ width: "100%" }}>
            ...
            <ActiveLastBreadcrumb label={"Welcome"} endpoint={"List Welcome"} />
            <Paper sx={{ width: "100%", mb: 2 }}>
                <Box sx={{ padding: '5px 16px' }}>
                    <Header name={"Welcomes"} description={"Manage your welcomes here."} />
                </Box>

                <Box sx={{ padding: '5px 16px' }}>
                    {hasPermission("Create Welcome") ? (
                        <Link to="/welcome/add" style={{ textDecoration: "none" }}>
                            <Button variant="contained">Create Welcome</Button>
                        </Link>
                    ) : (
                        ...
                        <TableBody>
                            {visibleRows.map((row) => (
                                <TableRow hover key={row.id}>
                                    <TableCell>
                                        <ActionEdit
                                            onEdit={() => navigate(`/welcome/edit/${row.id}`)}
                                            message="Edit Welcome"
                                            disabled={!hasPermission("Update Welcome")}
                                        />
                                        <ActionDelete
                                            link={`${import.meta.env.VITE_API_URL}/welcome/delete/${row.id}`}
                                            onDelete={fetchWelcome}
                                            message="Welcome deleted successfully"
                                            disabled={!hasPermission("Delete Welcome")}
                                        />
                                    </TableCell>
