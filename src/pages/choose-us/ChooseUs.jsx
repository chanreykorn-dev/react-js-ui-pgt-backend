...
import OptionChoose from "../../components/OptionChoose";

const headCells = [
    { id: "action", label: "Action", width: 120 },
    { id: "option", label: "Options", width: 120 },
    { id: "id", label: "ID", width: 60 },
    { id: "title", label: "Title", width: 280 },
    { id: "icon", label: "Icon", width: 150 },
    { id: "descriptions", label: "Description", width: 500 },
    { id: "category", label: "Category", width: 500 },
    { id: "category_sub", label: "Sub Category", width: 500 },
...
export default function ChooseUs() {
    ...
    const fetchCategories = async () => {
        try {
            ...
            const res = await fetch(`${import.meta.env.VITE_API_URL}/choose-us/`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            if (!res.ok) throw new Error("Failed to fetch choose us");
    ...
    return (
        <Box sx={{ width: "100%" }}>
            <ToastContainer
                ...
                transition={Bounce}
            />
            <ActiveLastBreadcrumb label={"Choose Us"} endpoint={"List Choose Us"} />
            <Paper sx={{ width: "100%", mb: 2 }}>
                <Box sx={{ padding: "5px 16px" }}>
                    <Header title="Choose Us" subtitle="List of Choose Us" />
                </Box>
                <Box sx={{ padding: "5px 16px" }}>
                    {hasPermission("Create choose us") ? (
                        <Link to="/choose-us/add">
                            <Button variant="contained">Create Choose Us</Button>
                        </Link>
                    ) : (
                        ...
                        <TableBody>
                            {visibleRows.map((row) => (
                                <TableRow hover key={row.id}>
                                    <TableCell>
                                        <ActionEdit
                                            onEdit={() => navigate(`/choose-us/edit/${row.id}`)}
                                            message="Edit Choose Us"
                                            disabled={!hasPermission("Update choose us")}
                                        />
                                        <ActionDelete
                                            link={`${import.meta.env.VITE_API_URL}/choose-us/delete/${row.id}`}
                                            onDelete={fetchCategories}
                                            message="Choose Us deleted successfully"
                                            disabled={!hasPermission("Delete choose us")}
                                        />
                                    </TableCell>
                                    <TableCell>
